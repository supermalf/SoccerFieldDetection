...
void BKPNeuralNet::CreateNet( int Layers, int *nLayers)
{
  ...
  _gradientsMt=(float***)malloc((Layers-1)*sizeof(float**));
	...
	_layers = Layers;
  _nLayers = new int[_layers];
  for( int i=0 ; i<_layers ; i++ )
    _nLayers[i] = nLayers[i];
  _flagMomentum = 0;
}
                            ...
                            float *correctPerc )
{
  ...
  int cPerc = 0;      //< Porcentagem das amostras que estão com saída "correta":
  // Executando os treinamentos obrigatórios:
  for( int thisSample=0 ; thisSample<nSamples ; thisSample++ )
  {
    float ret=0;
    Use( inSize, inMatrix[thisSample] );
    ret = OutputError(outMatrix[thisSample], outSize);
    // Caso em que a saída é 0.9f:
    if( outMatrix[thisSample][0] > 0.8f )
      if( _neurons[_nLayers[_layers-1]][0] < 0.5 )
        cPerc += 1;
    // Caso em que a saída é 0.1f:
    if( outMatrix[thisSample][0] < 0.2f )
      if( _neurons[_nLayers[_layers-1]][0] > 0.5 )
        cPerc += 1;

    //if( thisSample%100 == 0)
      //printf("%d %lf\n", thisSample, thisOutput);
    thisOutput += ret;
    //printf("%d %lf\n", thisSample, thisOutput);
  }
  // Retorna o erro médio quadrático:
  *retRMS_Error = (thisOutput/nSamples);
  // Retorna a porcentagem de acerto:
  *correctPerc = 1-(float)cPerc/(float)nSamples;
  return 0;
}
              ...
              float l_rate, float momentum, int* retExecutedTrains )
{
  ...
  while( StdDev > minStdDev )
  {
    ...
    if( (nTrains%numTrains) == 0 ) //< A cada numTrains treinamentos, testa o erro:
    {
      ...
      float mean = 0;
      float CorrectPerc = 0;
      RMS_error( inMatrix, outMatrix, inSize, outSize, nSamples, &retRMS_Error, &CorrectPerc);
...
int BKPNeuralNet::AutoTrain( float**inMatrix, float **outMatrix, int inSize, int outSize, int nSamples, 
                            int minTrains, int varVectorSize, float minStdDev, int numTrains, TrainType type, 
                            float l_rate, float momentum, int* retExecutedTrains, float *retRMSError, float *retBestPerc )
{
  // Casos de retorno:
  if( (!inMatrix) || (!outMatrix) || (inSize!=_nLayers[0]) || (_nLayers[_layers-1]!=outSize) )
    return -1;

  float* tmpIn = new float[_nLayers[0]];
  float* tmpOut = new float[_nLayers[_layers-1]];

  float thisOut[5] = { 0, 1, 2, 3, 4 };

  // melhor performance:
  float bestPerc = (float)MIN_VALUE;
  ...
  for( int i=0 ; i<minTrains ; i++ )
  {
    ...
    {
      thisSample = RandInt(0, (nSamples-1));
      while( outMatrix[thisSample][0] != eq )
        thisSample = RandInt(0, (nSamples-1));
      if( eq == 0.1f )
        eq = 0.9f;
      else if( eq == 0.9f )
    ...
    // Copiando a entrada atual para um vetor temporário:
    for( int f=0 ; f<_nLayers[0] ; f++ )
      tmpIn[f] = inMatrix[thisSample][f];
    // Copiando a saída atual para um vetor temporário:
    for( int f=0 ; f<_nLayers[_layers-1] ; f++ )
      tmpOut[f] = outMatrix[thisSample][f];

    Train( inSize, &tmpIn[0], outSize, &tmpOut[0], l_rate, momentum );
    //printf("%lf\t" , _neurons[_nLayers[_layers-1]][0] );
    //ShiftLeft( &thisOut[0], 5, _neurons[_nLayers[_layers-1]][0], 5 );
    //bool ret=false;

    //if( _neurons[_nLayers[_layers-1]][0] > MAX_VALUE || _neurons[_nLayers[_layers-1]][0] < MIN_VALUE )
      //ret = false;
    //else
      //ret = true;
  ...
  while( StdDev > minStdDev )
  {
    ...
    if( (nTrains%numTrains) == 0 ) //< A cada numTrains treinamentos, testa o erro:
    {
      ...
      float mean = 0;
      float CorrectPerc = 0;
      RMS_error( inMatrix, outMatrix, inSize, outSize, nSamples, &RMS_Error, &CorrectPerc );
      if( bestPerc < CorrectPerc )
        bestPerc = CorrectPerc;

      //printf("Porcentagem de acerto: %lf\n", bestPerc );

      if( RMS_Error <= 0 ) //< Caso a rede não tenha dado RMS_Error inválido, continua o procedimento:
        return -1;

      varFlag = ShiftLeft( varVector, varVectorSize, RMS_Error, ptVarVector );
      if( varFlag == true )
      {
        lastVariance = Variance( varVector, varVectorSize, &mean );
        StdDev = ((float)sqrt(lastVariance))/mean;
      ...
      ptVarVector++;
      // Armazena o último valor do erro RMS:
      *retRMSError = RMS_Error;
      // Armazena o último valor da porcentagem de acerto:
      *retBestPerc = bestPerc;

    }

    nTrains++;
    if( nTrains >= 50000 )   //< O número máximo de treinamentos será 150000. (BACALHAU!!! CONSERTAR DEPOIS!!!).
      StdDev = minStdDev;
...
/**
* Obtém a topologia da rede.
* @param layers Número de camadas da rede (retornada).
* @param nLayers Vetor (alocado internamente) de dimensão layers contendo o número de neurônios 
* em cada camada.
*/
void BKPNeuralNet::getTopology( int *layers, int **nLayers )
{
  int *tmpnLayers = new int[_layers];
  for( int i=0 ; i<_layers ; i++ )
    tmpnLayers[i] = _nLayers[i];
